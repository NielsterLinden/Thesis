...
- Run shell examples from the repository root (paths use `scripts/...` relative to it).

## Stage A — manifest

```bash
python scripts/wandb/eval_pipeline/stage_a_manifest.py \
  --raw-csv scripts/wandb/archive/wandb_cleanup_2026-04-29/backfill_snapshots/2026-04-29_raw/00_raw_export.csv \
  --out-dir scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN \
...
python scripts/wandb/eval_pipeline/stage_b_inference.py \
  --manifest scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN/00_eval_manifest.csv \
  --out-dir scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN \
  --row-start 0 \
  --row-count 100 \
  --limit 5
...
python scripts/wandb/eval_pipeline/merge_stage_b_shards.py \
  --phase-dir scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN
```
...
```bash
python scripts/wandb/eval_pipeline/stage_c_aggregate.py \
  --raw-csv scripts/wandb/archive/wandb_cleanup_2026-04-29/backfill_snapshots/2026-04-29_raw/00_raw_export.csv \
  --results scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN/01_eval_outcomes.csv \
  --out-dir scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN
```
...
```bash
python scripts/wandb/eval_pipeline/stage_d_push.py \
  --combined-csv scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN/02_eval_combined.csv \
  --out-dir scripts/wandb/eval_pipeline/snapshots/MY_PHASE2_RUN \
  --entity YOUR_ENTITY \
  --project thesis-ml
```
