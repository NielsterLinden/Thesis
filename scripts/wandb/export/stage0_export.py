#!/usr/bin/env python
"""Stage 0: export W&B runs to CSV using a fixed column schema.

Columns come from ``wandb_mcp_columns_thesis-ml.csv`` beside this script
(column_name, kind rows). Each data column is ``{kind}/{column_name}``.
"""
...
DEFAULT_SCHEMA = Path(__file__).resolve().parent / "wandb_mcp_columns_thesis-ml.csv"
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        help="Column schema: column_name,kind rows (default: beside stage0_export.py)",
    )
    ap.add_argument(
        "--out-csv",
        type=Path,
        default=default_snap,
        help="Output CSV path (default: scripts/wandb/export/snapshots/<today>_raw/00_raw_export.csv)",
    )
