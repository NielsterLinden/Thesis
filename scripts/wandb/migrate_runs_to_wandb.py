...
Usage:
    ...
    python scripts/wandb/migrate_runs_to_wandb.py --runs-dir /data/atlas/users/nterlind/outputs/runs --dry-run

    # Migrate all runs
    python scripts/wandb/migrate_runs_to_wandb.py --runs-dir /data/atlas/users/nterlind/outputs/runs

    # Migrate from both HPC and local
    python scripts/wandb/migrate_runs_to_wandb.py \
    ...
    python scripts/wandb/migrate_runs_to_wandb.py --sweep-dir /data/atlas/users/nterlind/outputs/multiruns/exp_*

    # Custom project/entity
    python scripts/wandb/migrate_runs_to_wandb.py --runs-dir ... --project my-project --entity my-team

    # Limit number of runs to migrate
    python scripts/wandb/migrate_runs_to_wandb.py --runs-dir ... --limit 10

    # Upload model artifacts to WandB (opt-in; default is no artifacts)
    python scripts/wandb/migrate_runs_to_wandb.py --sweep-dir ... --artifacts
"""
...
# Add src to path for imports (scripts/wandb/ -> project root)
...
def migrate_run(
    ...
    *,
    ...
    -------
    bool | str
        ``True`` if dry-run would succeed; ``"exists"`` if display name already in W&B;
        new run id string after successful upload; ``False`` on failure.
    """
    run_name = run_dir.name
    wandb_display_name = f"{run_name}{name_suffix}" if name_suffix else run_name

    # === Check if already exists in WandB ===
    if existing_runs is not None and wandb_display_name in existing_runs:
        return "exists"
    ...
    if name_suffix:
        wandb_config["recovery/name_suffix"] = name_suffix
        wandb_config["recovery/facts_run_folder"] = run_name
    if extra_config:
    ...
    if dry_run:
        ...
        logger.info(
            "[DRY RUN] Would migrate: %s as W&B name %s (%d epochs, %s%s)%s%s",
            run_name,
            wandb_display_name,
            n_epochs,
            model_type,
    ...
    try:
        ...
        new_id = run.id
        logger.info("W&B new run id=%s url=%s", new_id, run.url)
        ...
        if upload_artifacts:
            ...
            if model_path.exists():
                art = wandb.Artifact(f"model-{wandb_display_name}", type="model", metadata=artifact_metadata)
                art.add_file(str(model_path))
                wandb.log_artifact(art)
                logger.info("Uploaded artifact: model-%s", wandb_display_name)
            ...
            if json_model_path.exists() and not model_path.exists():
                ...
                wandb.log_artifact(art)
                logger.info("Uploaded artifact: model-%s (json)", wandb_display_name)

        run.finish()
        model_type = wandb_config.get("model/type", "unknown")
        logger.info(
            "Migrated: %s -> W&B %s id=%s (%d epochs, %s)",
            run_name,
            wandb_display_name,
            new_id,
            n_epochs,
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help=("Append to W&B display name (not the on-disk folder name). " "Use e.g. _recovered when re-uploading facts after a broken cloud run id, " "so the new upload does not collide with duplicate-name skipping."),
    )
