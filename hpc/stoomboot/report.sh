...
OUTPUT_DIR="/data/atlas/users/nterlind/logs/report_${CLUSTER_ID}"
mkdir -p "$OUTPUT_DIR"

# Try to move files, with error handling
if mv "/data/atlas/users/nterlind/logs/report_${CLUSTER_ID}.out" "${OUTPUT_DIR}/report_${CLUSTER_ID}.out" 2>/dev/null; then
    echo "Moved .out file successfully"
else
    echo "Warning: Could not move .out file (may still be in use)"
fi

if mv "/data/atlas/users/nterlind/logs/report_${CLUSTER_ID}.err" "${OUTPUT_DIR}/report_${CLUSTER_ID}.err" 2>/dev/null; then
    echo "Moved .err file successfully"
else
    echo "Warning: Could not move .err file (may still be in use)"
fi

if mv "/data/atlas/users/nterlind/logs/report_${CLUSTER_ID}.log" "${OUTPUT_DIR}/report_${CLUSTER_ID}.log" 2>/dev/null; then
